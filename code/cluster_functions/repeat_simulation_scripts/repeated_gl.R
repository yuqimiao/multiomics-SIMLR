...
for(i in 1:repeat_time){
  ...
  gl_result = GL_SIMLR_tunning(data_list = data_list,c = 4,gamma_grid = NA, rho_grid = seq(0.01,0.1, by = 0.01))$best_tunn[1,]
  hyper_res[[i]] = gl_result$tunning_all
  ...
  snf_res = SNF(aff_mat)
  cluster_snf = spectralClustering(snf_res,K = 4)
...
save(hyper_res, file = paste("./simulation_gl/",scenario_name[[n]],"_hyper",".Rdata", sep = ""))
