...
effective_size = list(c(1,3,6),c(2,3,5),c(5,5,5)) # 3
...
for(i in 1:repeat_time){
  ## gl_simlr
  sim = simulation_3(eff_size = par[[n]]$eff_size, sub_ratio = par[[n]]$sub_ratio, dist = par[[n]]$distribution,sigma = rep(10,3) )
...
eva_table = save(eva_table, file = paste("./simulation_gl/simulation_gl_var10",scenario_name[[n]],".Rdata", sep = ""))
