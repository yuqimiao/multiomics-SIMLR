...
data_list = list(scen1,scen2,scen3,scen4,scen5,scen6,scen7,scen8)
names(data_list) = c('scen1','scen2','scen3','scen4','scen5','scen6','scen7','scen8')
...
load("simu_0217/best_par1-9.Rdata")
...
save(res_gl, file = paste("simu_0217/res_gl",n, ".Rdata", sep = ""))
...
res_gl_umkl = lapply(names(mk_list), FUN = function(x){
  par1 = best_par_all[[x]]
  nmi_f = 0
  nmi_ls_tmp = NULL
  m = 1
...
save(res_gl_umkl, file = paste("simu_0217/res_gl_umkl",n, ".Rdata", sep = ""))

## evaluate
nmi_gl_umkl_ls = map_dbl(res_gl_umkl, function(x){
  cluster = spectralClustering(as.matrix(x$S), K = 4)
  nmi =igraph::compare(rep(1:4, each = 50), cluster, method = "nmi")
  nmi
})
unmatched_mk = names(mk_list)[-match(names(nmi_gl_umkl_ls),names(mk_list))]
for(i in unmatched_mk){
  nmi_gl_umkl_ls[[i]] = NA
}
...
save(res_cim, file = paste("simu_0217/res_cim",n, ".Rdata", sep = ""))
...
eva_table = as_tibble(cbind(nmi_gl_ls,nmi_gl_umkl_ls,nmi_cim_ls)) %>%
  mutate(scenario = c('scen1','scen2','scen3','scen4','scen5','scen6','scen7','scen8'),
         w1_gl = map_dbl(res_gl, .f = function(x) {sum(x$w[1:22])}),
         w2_gl = map_dbl(res_gl, .f = function(x) {sum(x$w[23:44])}),
         w1_gl_umkl = map_dbl(res_gl_umkl, .f = function(x) {sum(x$w[1:22])}),
         w2_gl_umkl = map_dbl(res_gl_umkl, .f = function(x) {sum(x$w[23:44])}),
...
save(eva_table, file = paste("simu_0217/eva_table", n,".Rdata", sep = ""))
