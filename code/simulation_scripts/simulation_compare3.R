...
scenario_name = c(paste("sub1111", c(paste("eff111", paste("sig",sigma_ls, sep = ""),sep = "_"),
                                     paste("eff133", paste("sig",sigma_ls, sep = ""),sep = "_"),
                                     paste("eff135", paste("sig",sigma_ls, sep = ""),sep = "_")),
                        sep = "_"),
                  paste("sub1234", c(paste("eff111", paste("sig",sigma_ls, sep = ""),sep = "_"),
                                     paste("eff135", paste("sig",sigma_ls, sep = ""),sep = "_"),
                                     paste("eff135", paste("sig",sigma_ls, sep = ""),sep = "_")),
                        sep = "_")
)
...
for(m in 1:length(par)){
  ...
  ##gl-simlr
  W_g = GL_SIMLR_4(data_list = list(t(data$data1),t(data$data2),t(data$data3)), c = 4)
  W_g_nbeta = GL_SIMLR_4(data_list = list(t(data$data1),t(data$data2),t(data$data3)), c = 4, beta = 1)
  W_g_tunn = GL_SIMLR_tunning(data_list = list(t(data$data1),t(data$data2),t(data$data3)),
                              c = 4,
                              gamma_grid = NA,
                              # rho_grid = 0.1,
                              rho_grid = c(seq(0.01,0.09, by = 0.01), seq(0.1,1,0.1), seq(1,10,1)),
  ...
  cluster_spec_gn = W_g_nbeta$cluster
  cluster_spec_gt = W_g_tunn$res_opt$cluster
  #evaluation
  eva_table_nmi = tibble(scenario = scenario_name[m],
                         ...
                         nmi_spec_gn = compare(truth, cluster_spec_gn, method = "nmi"),
                         nmi_spec_gt = compare(truth, cluster_spec_gt, method = "nmi")
                        )
  eva_table_rand = tibble(scenario = scenario_name[m],
                          ...
                          rand_spec_gn = compare(truth, cluster_spec_gn, method = "rand"),
                          rand_spec_gt = compare(truth, cluster_spec_gt, method = "rand")
  )
...
#
save(res,
     file = paste("simu_all/sim_res",n,".Rdata", sep = ""))
