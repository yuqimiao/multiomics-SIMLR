SIMLR_no_weight = function(distX, c, no.dim = NA, k = 10, cores.ratio = 1 ) {
...
SIMLR_multi_fix_weight = function(D_Kernels, weight_list, w_data, c, no.dim = NA, k = 10, cores.ratio = 1 ) {
  ...
  r = -1
  beta = 0.8
  ## get the fixed weight combination
  weight = NULL
  for (i in 1:length(weight_list)){
    weight = c(weight, weight_list[[i]]*w_data[i])
  }

  ## get the fixed distance
  distX = array(0,c(dim(D_Kernels[[1]])[1],dim(D_Kernels[[1]])[2]))
  for (i in 1:length(D_Kernels)) {
    distX = distX + D_Kernels[[i]]* weight[i]
  }
...
weight_kernel = function(data_list,dist_list = NA, trans = "extreme"){
  ## construct kernels
  dist_list = dist_list
  if(is.na(dist_list)){
    dist_kernel_list = lapply(data_list, FUN = function(x) D_kernels(x_fun = x) )
  }else(
    dist_kernel_list = lapply(dist_list, FUN = function(x) D_kernels(Diff_fun = dist_list) )
  )
  kernel_list = lapply(dist_kernel_list, FUN = function(x){x[[1]]})
  D_kernels = lapply(dist_kernel_list, FUN = function(x){x[[2]]})
  ## calculate square of rank normalization,
  if(trans == "extreme"){
    rank2_list = lapply(kernel_list, FUN = function(x) {
      t(apply(x,MARGIN = 1, function(y){(RNOmni::rankNorm(y))^2}))
    })}else if(trans == "large"){
      rank2_list = lapply(kernel_list, FUN = function(x) {
  ...
  D_kernel_w = Map("*",ind_weight, D_kernels)
  return(D_kernel_w)
}

SIMLR_multi_generated_weight = function(data_list, c, no.dim = NA, k = 10, cores.ratio = 1 ) {

  #set any required parameter to the defaults
  if(is.na(no.dim)) {
    no.dim = c
  }
  D_Kernels = weight_kernel(data_list,trans = "extreme")
  # # start the clock to measure the execution time
  ptm = proc.time()
  #
  # set some parameters
